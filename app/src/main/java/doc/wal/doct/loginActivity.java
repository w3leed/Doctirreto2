            ...
            if(TextUtils.isEmpty(doctorPassword)){
                ...
                if(networkInfo == null){
                    ...
                    if(cursor.getCount() == 0){
                        ...
                        cursor.moveToFirst();
                        String userNa = cursor.getString(cursor.getColumnIndex(doctorContract.doctorEntry.COLUMN_EMAIL));
                        final String pushId = cursor.getString(cursor.getColumnIndex(doctorContract.doctorEntry.COLUMN_PUSHID));
                ...
                for (DataSnapshot appleSnapshot: dataSnapshot.getChildren()) {
                    ...
                    for(DataSnapshot appleSnapshot1: appleSnapshot.getChildren()){
                        ...
                        if(appleSnapshot1.getKey().equals(charUtility.filterString(doctorUsername))){
                            ...
                            cv.put(doctorContract.doctorEntry.COLUMN_PUSHID, appleSnapshot.getKey());
                            cv.put(doctorContract.doctorEntry.COLUMN_NAME, firstname.concat("@@@@").concat(lastname));
                            cv.put(doctorContract.doctorEntry.COLUMN_PHONE_NUMBER, userphone);
                            cv.put(doctorContract.doctorEntry.COLUMN_EMAIL, useremail);
                            cv.put(doctorContract.doctorEntry.COLUMN_PASSWORD, userpassword);
                            cv.put(doctorContract.doctorEntry.COLUMN_TITLE, title);
                            cv.put(doctorContract.doctorEntry.COLUMN_INSTITUTE, doctorInstitute);
                            cv.put(doctorContract.doctorEntry.COLUMN_IMAGE, (byte[]) null);
                            cv.put(doctorContract.doctorEntry.COLUMN_INSTITUTE_ADDRESS, doctorInstituteAddress);

                            Uri uri = getContentResolver().insert(doctorContract.doctorEntry.CONTENT_URI, cv);
                            if(uri == null){
                                progressDialog.dismiss();
                                return;
                            }else {
