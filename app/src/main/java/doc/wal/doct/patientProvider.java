...
public class patientProvider extends ContentProvider {
    ...
    public Cursor query(@NonNull Uri uri, @Nullable String[] projection, @Nullable String selection, @Nullable String[] selectionArgs, @Nullable String sortOrder) {
        ...
        switch (match){
            case PATIENTS:
                cursor = db.query(doctorContract.patientEntry.tableName(doctorPreference.getUsernameFromSP(MyApplication.getAppContext())), projection, selection, selectionArgs, null, null, sortOrder);
                break;
            case PATIENT_ID:
                List list4 = uri.getPathSegments();
                selection = doctorContract.patientEntry.COLUMN_PUSH_ID + "=?";
            ...
            case PATIENT_NAME:
                ...
                selection = doctorContract.patientEntry.COLUMN_NAME + " LIKE ?";
            ...
            case DOCTOR_ID:
                ...
                selection = doctorContract.doctorEntry.COLUMN_EMAIL + "=?";
                selectionArgs = new String[]{username};

                Cursor cursor1 = db.query(doctorContract.doctorEntry.TABLE_NAME, projection, selection, selectionArgs, null, null, sortOrder);

                if(cursor1.getCount() == 0){
                    return cursor1;
                    //Do Nothing
                }else {
                    selection = doctorContract.doctorEntry.COLUMN_PASSWORD + "=?";
                    selectionArgs = new String[]{password};

                    cursor1.moveToFirst();

                    Cursor cursor2 = db.query(doctorContract.doctorEntry.TABLE_NAME, projection, selection, selectionArgs, null, null, sortOrder);
            ...
            case SPECIFIC_DOCTOR:
                ...
                cursor = db.query(doctorContract.doctorEntry.TABLE_NAME, projection, selection, selectionArgs, null, null, sortOrder);
                break;
            default:
    ...
    public Uri insertPatient(Uri uri, ContentValues values){
        ...
        Long returnedId = db.insert(doctorContract.patientEntry.tableName(doctorPreference.getUsernameFromSP(MyApplication.getAppContext())), null, values);
        if(returnedId == -1){
            return null;
        }
    ...
    public Uri insertDoctor(Uri uri, ContentValues values){
        ...
        Long returnedId = db.insert(doctorContract.doctorEntry.TABLE_NAME, null, values);
        if(returnedId == -1){
            return null;
        }
        getContext().getContentResolver().notifyChange(uri, null);
    ...
    public int delete(@NonNull Uri uri, @Nullable String selection, @Nullable String[] selectionArgs) {
        ...
        switch (match) {
            case PATIENTS:
                ...
                getContext().getContentResolver().notifyChange(uri, null);
                return database.delete(doctorContract.patientEntry.tableName(doctorPreference.getUsernameFromSP(MyApplication.getAppContext())), selection, selectionArgs);
            case PATIENT_ID:
    ...
    public int update(@NonNull Uri uri, @Nullable ContentValues values, @Nullable String selection, @Nullable String[] selectionArgs) {
        ...
        switch (match) {
            case PATIENT_ID:
                ...
                return database.update(doctorContract.patientEntry.tableName(doctorPreference.getUsernameFromSP(MyApplication.getAppContext())), values, selection, selectionArgs);
            ...
            case SPECIFIC_DOCTOR:
                ...
                return database1.update(doctorContract.doctorEntry.TABLE_NAME, values, selection, selectionArgs);
